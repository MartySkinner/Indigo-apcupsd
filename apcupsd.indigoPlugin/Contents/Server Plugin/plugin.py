...
class Plugin(indigo.PluginBase):
    ...
    def runConcurrentThread(self):
        ...
        try:
            while True:
                ...
                if self.secondsBetweenUpdateChecks > 0:
                        ...
                        if timeNow > self.nextUpdateCheck:
                                ...
                                self.log.log(3, dbFlg, u"# of seconds between update checks: %s" % (int(self.secondsBetweenUpdateChecks)), self.logName)
                ...
                for dev in indigo.devices.iter(self.pluginId):
                    if dev.enabled:
                        devCount += 1
                        self.log.log(2, dbFlg, u"%s: Got device %s from Indigo" % (funcName, dev.name), self.logName)
                        self.readApcupsd(dev)
                        self.log.log(3, dbFlg, u"%s: Read device %s" % (funcName, dev.name), self.logName)

                if devCount == 0 and self.logLevel > 0:
                    self.log.log(2, dbFlg, u"%s: Completed device poll. No enabled devices found" % (funcName), self.logName)
                else:
                    if devCount == 1:
                        devWord = u"device"
                    else:
                        devWord = u"devices"
                    self.log.log(2, dbFlg, u"%s: Completed device poll. %s %s found" % (funcName, devCount, devWord), self.logName)
            ...
            self.log.log(2, dbFlg, u"%s: StopThread is now True" % (funcName), self.logName)
            stopEventServer(self)
    ...
    def readApcupsd(self, dev, parseOnly=False, tmpProps=False):
        ...
        dbFlg = False
        self.log.log(2, dbFlg, u"%s called" % (funcName), self.logName)
        self.log.log(4, dbFlg, u"%s: Received device:\n%s\n" % (funcName, unicode(dev)), self.logName)

        if tmpProps is False:
                try:
                        sAddress = dev.pluginProps["apcupsdAddress"]
                except KeyError:
                        self.log.logError(u"%s: Trying to get status for a device that is not configured" % (funcName), self.logName)
                        self.log.log(2, dbFlg, u"%s: Completed" % (funcName), self.logName)
                        return False
        ...
        self.log.log(4, dbFlg, u"%s: doing '%s status' for address %s on port %s" % (funcName, self.utilityBinary, sAddress, sPort), self.logName)
        ...
        while not apcupsdSuccess and apcupsdRetries < 5:
            if apcupsdRetries == 0:
                self.log.log(3, dbFlg, u"%s: starting read. Retries=%s" % (funcName, apcupsdRetries), self.logName)
            else:
                self.log.log(2, dbFlg, u"%s: starting read. Retries=%s" % (funcName, apcupsdRetries), self.logName)
            ...
            if result:
                self.log.logError(u"%s: Connection to %s failed with error code %s. Attempt %s of 5" % (funcName, self.utilityBinary, result, apcupsdRetries), self.logName)
                self.log.logError(u"%s: Returned output: %s" % (funcName, report), self.logName)
                apcupsdSuccess = False
                self.sleep(1)
            else:
                self.log.log(4, dbFlg, u"%s: report\n%s" % (funcName, report), self.logName)
                ...
                for line in report.split('\n'):
                    ...
                    if key != '':
                        metrics[key] = val
                        self.log.log(4, dbFlg, u"%s: parsed key=%s and val=%s" % (funcName, key, val), self.logName)
                ...
                if 'status' in metrics:
                    ...
                    if metrics['status'] == 'COMMLOST' and not self.apcupsdCommError:
                        ...
                        for state in dev.states:
                            ...
                            self.log.log(2, dbFlg, u"%s: changing state for: %s to n/a" % (funcName, state), self.logName)

                        self.log.log(3, dbFlg, u"%s: COMMLOST" % (funcName), self.logName)
                    elif metrics['status'] != 'COMMLOST' and self.apcupsdCommError:
                        dev.setErrorStateOnServer(None)
                        self.apcupsdCommError = False
                        self.log.log(3, dbFlg, u"%s: ONLINE" % (funcName), self.logName)
                else:
                        self.log.log(2, dbFlg, u"%s: Completed" % (funcName), self.logName)
            ...
            for metric in metrics:
                ...
                try:
                    if metric in dev.states:
                        self.log.log(4, dbFlg, u"%s: found metric: %s " % (funcName, metric), self.logName)

                        if self.apcupsdCommError:
                            if metric in self.commLostStatesList:
                                self.log.log(3, dbFlg, u"%s: found commLostStates: %s in list:%s" % (funcName, metric, self.commLostStatesList), self.logName)
                                try:
                        ...
                        self.log.log(4, dbFlg, u"%s: metric:%s, val:%s, is Error:%s" % (funcName, metric, value, self.apcupsdCommError), self.logName)
                except Exception as e:
                    self.log.logError(u"%s: error writing device state" % (funcName), self.logName)
                    e1 = sys.exc_info()[0]
                    self.log.logError(u"%s: Errors %s and %s" % (funcName, e, e1), self.logName)

            self.log.log(2, dbFlg, u"%s: Completed readings update from device: %s" % (funcName, dev.name), self.logName)
        else:
            self.log.logError(u"%s: Failed to get status for UPS %s after %s tries. Will retry in %s minutes" % (funcName, dev.name, apcupsdRetries, self.apcupsdFrequency), self.logName)
        self.log.log(2, dbFlg, u"%s: Completed" % (funcName), self.logName)
    ...
    def deviceStartComm(self, dev):
        funcName = inspect.stack()[0][3]
        dbFlg = False
        self.log.log(2, dbFlg, u"%s called for device %s" % (funcName, dev.name), self.logName)
...
#                pass
        ...
        if madeChanges is True:
            self.log.log(1, dbFlg, u"%s: Device %s updated to version %s" % (funcName, dev.name, k_deviceUpdateVersion), self.logName)
            tmpProps['version'] = k_deviceUpdateVersion
            dev.replacePluginPropsOnServer(tmpProps)

        if dev.enabled and not self.startingUp:
            self.log.log(2, dbFlg, u"%s: Resetting read loop to include device %s" % (funcName, dev.name), self.logName)
            self.readLoop = False
    ...
    def deviceStopComm(self, dev):
        ...
        self.log.log(2, dbFlg, u"%s: Resetting read loop to drop device %s" % (funcName, dev.name), self.logName)
        self.readLoop = False
    ...
    def didDeviceCommPropertyChange(self, origDev, newDev):
        funcName = inspect.stack()[0][3]
        dbFlg = False
        self.log.log(2, dbFlg, u"%s called for device %s" % (funcName, origDev.name), self.logName)

        self.log.log(4, dbFlg, u"%s: origDev:\n%s\n\nnewDev:\n%s\n" % (funcName, origDev, newDev), self.logName)

        if (origDev.pluginProps != newDev.pluginProps):
            self.log.log(2, dbFlg, u"%s: Resetting read loop to include device %s" % (funcName, newDev.name), self.logName)
            self.readLoop = False
    ...
    def triggerStartProcessing(self, trigger):
        funcName = inspect.stack()[0][3]
        dbFlg = False
        self.log.log(2, dbFlg, u"%s called for trigger %s" % (funcName, trigger.name), self.logName)

        self.log.log(4, dbFlg, u"%s: Received trigger:%s" % (funcName, trigger), self.logName)
        ...
        self.log.log(2, dbFlg, u"%s trigger %s started" % (funcName, trigger.name), self.logName)
        self.log.log(2, dbFlg, u"%s: Completed" % (funcName), self.logName)
    ...
    def triggerStopProcessing(self, trigger):
        ...
        if trigger.id in self.triggerDict:
            self.log.log(2, dbFlg, u"%s trigger %s found" % (funcName, trigger.name), self.logName)
            del self.triggerDict[trigger.id]

        self.log.log(2, dbFlg, u"%s trigger %s deleted" % (funcName, trigger.name), self.logName)
        self.log.log(2, dbFlg, u"%s: Completed" % (funcName), self.logName)
    ...
    def triggerEvent(self, eventId, devId):
        ...
        dbFlg = False
        self.log.log(2, dbFlg, u"%s called" % (funcName), self.logName)
        self.log.log(3, dbFlg, u"%s: received trigger %s for device %s" % (funcName, eventId, devId), self.logName)

        try:
            for trigId in self.triggerDict:
                self.log.log(3, dbFlg, u"%s: found trigger ID %s" % (funcName, trigId), self.logName)
                trigger = indigo.triggers[trigId]
                self.log.log(3, dbFlg, u"%s: found trigger %s" % (funcName, trigger), self.logName)
                device = self.triggerDict[trigId]
                if trigger.pluginTypeId == eventId and trigger.pluginProps['indigoDevice'] == device:
                    self.log.log(2, dbFlg, u"%s: matched trigger ID %s" % (funcName, trigId), self.logName)
    ...
    def buildAction(self, event):
        ...
        try:
            ...
            self.log.log(3, dbFlg, u"%s: Received event %s for device %s" % (funcName, event, devid), self.logName)
        except ValueError:
            self.log.logError(u"%s: Received incorrectly formatted event %s, should be DEVNAME_OR_ID%sEVENTTYPE" % (funcName, event, k_eventServerSeparator), self.logName)
            return

        if event in k_eventServerEvents:
            self.log.log(3, dbFlg, u"%s: Validated event %s for device %s" % (funcName, event, devid), self.logName)
        else:
            self.log.logError(u"%s: Received bad event type %s for device %s" % (funcName, event, devid), self.logName)
            return
        ...
        try:
                ...
                self.log.logError(u"%s: Unrecognized device name or ID %s for event %s" % (funcName, devid, event), self.logName)
                return
        # check now to see if specified device is really a device created by this plugin...
        if dev.pluginId != self.pluginId:
                self.log.logError(u"%s: Device name or ID %s is not associated with this plugin for event %s" % (funcName, devid, event), self.logName)
                return
        ...
        self.log.log(4, dbFlg, u"%s: built action: \n%s and retrieved dev: \n%s" % (funcName, action, dev), self.logName)
    ...
    def actionControlApcupsd(self, action, dev):
        ...
        self.log.log(2, dbFlg, u"%s: Entered for dev: %s, and action: %s" % (funcName, dev.name, action.description), self.logName)
        self.log.log(4, dbFlg, u"%s: Received action: \n%s\n and dev:\n%s" % (funcName, action, dev), self.logName)
        ...
        try:
           apcupsdAction = action.props['actionType']
            ...
            apcupsdAction = ''
        self.log.log(2, dbFlg, u"%s: found deviceId=%s, actionType=%s" % (funcName, action.deviceId, apcupsdAction), self.logName)
        ...
        if action.pluginTypeId == 'readApcupsd':
            ...
            self.log.logError(u"%s: unknown action %s requested" % (funcName, apcupsdAction), self.logName)
        ...
        for key in valuesDict:
            if key.find('apcupsdState') != -1:
                try:
                        ...
                        self.log.log(3, dbFlg, u"%s: using '%s' default of %s" % (funcName, key, self.defaultStatesDict[key]), self.logName)
                except KeyError:
                        self.log.log(3, dbFlg, u"%s: missing entry '%s' in defaultStates.dict, defaulting to False" % (funcName, key), self.logName)
                        defaultsState = False
    ...
    def selectQueryDevice(self, valuesDict, typeId, devId):
        ...
        self.log.log(4, dbFlg, u"%s: temporary properties for new device\n%s\n" % (funcName, tmpProps), self.logName)
                ...
                for metric in metrics:
                        ...
                        try:
                                if key in valuesDict:
                                        self.log.log(3, dbFlg, u"%s: found matching state: %s" % (funcName, metric), self.logName)
                                        valuesDict[key] = True
                        except:
                                pass
        else:
                self.log.log(1, dbFlg, u"%s: Cannot determine which states the device might provide" % (funcName), self.logName)
        ...
        for key in dev.pluginProps:
            if key.find('apcupsdState') != -1:
                if dev.pluginProps[key]:
                    ...
                    self.log.log(4, dbFlg, u"%s: %s added as state" % (funcName, key[12:]), self.logName)
            else:
                self.log.log(4, dbFlg, u"%s: %s property is not a state" % (funcName, key[12:]), self.logName)

        self.log.log(3, dbFlg, u"%s: returning statesList: %s" % (funcName, statesList), self.logName)
